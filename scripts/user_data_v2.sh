...
db_consistency_check() {
  ...
  if [ $USER_CHECK -ne 0 ] || [ $DB_CHECK -ne 0 ]; then
    ...
    mariadb -h $${DB_EP} -P 3306 -u admin -p"$${ADMIN_PWD}" --ssl-verify-server-cert  --ssl-ca=/root/eu-west-1-bundle.pem < /var/app/bootstrap.sql || echo "DB user/db Creation Failed!!!!; exit 1"
  else
